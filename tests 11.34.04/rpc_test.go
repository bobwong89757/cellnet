...
func rpc_StartServer() {
	...
	proc.BindProcessorHandler(rpc_Acceptor, "tcp.ltv", func(ev cellnet.Event) {
		...
		case *TestEchoACK:
			log.GetLog().Debugf("server recv rpc ", *msg)
	...
	case *cellnet.SessionConnected:
		for i := 0; i < 2; i++ {
			...
			go func(id int) {
				...
				log.GetLog().Debugf("client sync recv:", msg.Msg, id*100)
	...
	case *cellnet.SessionConnected:
		for i := 0; i < 2; i++ {
			...
			rpc.Call(ev.Session(), &TestEchoACK{
				...
				case *TestEchoACK:
					log.GetLog().Debugf("client sync recv:", v.Msg)
	...
	case *cellnet.SessionConnected:
		for i := 0; i < 2; i++ {
			...
			rpc.CallSyncType(ev.Session(), &TestEchoACK{
				...
				log.GetLog().Debugf("client type sync recv:", ack)
